...
A selected ``FOURTH``-category subject contributes its points above 2.00 to
GPA, remains outside the GPA denominator and main pass/fail rule, but is
included in displayed total/full/percentage and merit total marks.
"""
...
class FourthSubjectResultTests(TestCase):
    ...
    def test_fourth_bonus_is_capped_and_fourth_failure_does_not_fail_main_result(self):
        ...
        # The fourth paper is visible and increases total/full/percentage, but
        # the GPA denominator remains the ten main papers.
        self.assertEqual((
            capped['total_obtained'], capped['total_full'], capped['percentage'], capped['main_subject_count'],
        ), (
            Decimal('860'), Decimal('1100'), 78.18, 10,
        ))
        ...
        self.assertEqual((
            agriculture_row['is_fourth_subject'],
            agriculture_row['counts_toward_main_gpa'],
            agriculture_row['counts_toward_total'],
        ), (True, False, True))

        # An F in Agriculture is printed as F, gives no bonus, but does not
        # turn the ten-main-subject result into Fail. Its marks/full marks do
        # remain in the displayed aggregate.
        self.assertEqual((
            fourth_failed['gpa'], fourth_failed['status'], fourth_failed['total_obtained'],
            fourth_failed['total_full'], fourth_failed['percentage'],
        ), (Decimal('4.80'), 'Pass', Decimal('810'), Decimal('1100'), 73.64))
        ...
        self.assertEqual((
            elective_row['is_fourth_subject'],
            elective_row['counts_toward_main_gpa'],
            elective_row['counts_toward_total'],
        ), (False, True, True))
        self.assertEqual((
            ordinary_optional['main_subject_count'], ordinary_optional['total_obtained'], ordinary_optional['total_full'],
        ), (11, Decimal('950'), Decimal('1200')))
...
class FourthSubjectMeritTieBreakTests(TestCase):
    """Pin grade -> uncapped GPA -> total -> roll as unique merit order."""

    @classmethod
    def setUpTestData(cls):
        institution = Institution.objects.create(name='Fourth Subject Merit School', classes='9')
        cls.exam = Exam.objects.create(
            name='Fourth Subject Merit-2026', exam_type='FINAL_TERM', institution=institution,
            admission_class='9', session='2026', is_published=True,
        )
        cls.main_one = Subject.objects.create(code='R4M1', name='Merit Main One', full_marks=100)
        cls.main_two = Subject.objects.create(code='R4M2', name='Merit Main Two', full_marks=100)
        cls.fourth = Subject.objects.create(
            code='R4F1', name='Merit Fourth', full_marks=100, category='FOURTH',
        )
        for subject in (cls.main_one, cls.main_two):
            SubjectRequirement.objects.create(
                institution=institution, admission_class='9', subject=subject, requirement_type='MANDATORY',
            )
        fourth_requirement = SubjectRequirement.objects.create(
            institution=institution, admission_class='9', subject=cls.fourth,
            requirement_type='OPTIONAL', optional_set_key='merit-fourth',
        )
        cls.low_roll = Student.objects.create(
            institution=institution, student_id='R4002', name='Same Score Lower Roll',
            admission_class='9', section='A', roll_no=2, admission_year=2026,
        )
        cls.high_roll = Student.objects.create(
            institution=institution, student_id='R4010', name='Same Score Higher Roll',
            admission_class='9', section='A', roll_no=10, admission_year=2026,
        )
        cls.lower_raw_gpa = Student.objects.create(
            institution=institution, student_id='R4001', name='Higher Total Lower Formula GPA',
            admission_class='9', section='A', roll_no=1, admission_year=2026,
        )
        students = (cls.low_roll, cls.high_roll, cls.lower_raw_gpa)
        StudentSubjectChoice.objects.bulk_create([
            StudentSubjectChoice(student=student, requirement=fourth_requirement) for student in students
        ])
        marks = []
        for student in (cls.low_roll, cls.high_roll):
            marks.extend([
                ExamMark(exam=cls.exam, student=student, subject=cls.main_one, marks_obtained=80),
                ExamMark(exam=cls.exam, student=student, subject=cls.main_two, marks_obtained=80),
                ExamMark(exam=cls.exam, student=student, subject=cls.fourth, marks_obtained=80),
            ])
        # Still A+/5.00 displayed, and a larger total (250), but an uncapped
        # formula GPA of 6.00 loses to 6.50 before total-marks comparison.
        marks.extend([
            ExamMark(exam=cls.exam, student=cls.lower_raw_gpa, subject=cls.main_one, marks_obtained=80),
            ExamMark(exam=cls.exam, student=cls.lower_raw_gpa, subject=cls.main_two, marks_obtained=70),
            ExamMark(exam=cls.exam, student=cls.lower_raw_gpa, subject=cls.fourth, marks_obtained=100),
        ])
        ExamMark.objects.bulk_create(marks)

    def test_a_plus_ties_use_uncapped_formula_gpa_then_total_then_lowest_roll(self):
        _columns, results = build_exam_results(self.exam)
        ranked = [row for row in results if row['status'] == 'Pass']

        self.assertEqual(
            [(row['student'].roll_no, row['position']) for row in ranked],
            [(2, 1), (10, 2), (1, 3)],
        )
        self.assertEqual(
            [(row['grade'], row['gpa'], row['ranking_gpa'], row['total_obtained']) for row in ranked],
            [
                ('A+', Decimal('5.00'), Decimal('6.50'), Decimal('240')),
                ('A+', Decimal('5.00'), Decimal('6.50'), Decimal('240')),
                ('A+', Decimal('5.00'), Decimal('6.00'), Decimal('250')),
            ],
        )
