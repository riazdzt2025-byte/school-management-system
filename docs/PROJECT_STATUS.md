...
_Last updated: ২০২৬-০৯-২০ (EX-03 — Group-based Mark Evaluation, ৬৬৮ Django + ১৪ Node)_\
_Base commit: `7a13e33` (Merge PR #41) on branch `arena/01a0bd0d-school-management-system` — equals `origin/main`_\
_Working tree: clean, no local overwrite, no reset --hard, no git clean_

> **২০২৬-০৯-২০ EX-03 পরবর্তী (প্রম্পট ০৪/২৮):** এই checkout-এ নিজে চালিয়ে যাচাই — **৬৬৮ Django + ১৪ Node test pass** (৬২৫→৬৩৮→৬৫৩→৬৬৮ ক্রমে EX-01/02/03), `check` 0 issue, `check --deploy` ৬টি dev warning, `makemigrations --check` clean (leaf **0044** — EX-03 group migration), backup drill sqlite ১৫/০ ও moto-S3 ২৮/০ pass। EX-01/EX-02/EX-03 সব Complete — প্রমাণ `docs/prompts/reports/০০-baseline.md` + `EX-01.md`/`EX-02.md`/`EX-03.md` (§20-এ সারসংক্ষেপ)। নিচের ২০২৬-০৯-১৭-এর section-গুলো ঐতিহাসিক রেকর্ড; যেখানে সংখ্যা বা verdict ভিন্ন, §20-ই সত্য।

**Bengali TL;DR (সর্বশেষ — ২০২৬-০৯-২০ EX-03):** EX-03 **Group-based Mark Evaluation** implement হলো — `SubjectMarkSetting.group` (migration **0044**, blank=default, 9-12 only, exact/ MID owner সিদ্ধান্ত) + resolution chain **group→blank→Subject** (marks entry/import/result group-aware) + `mark_evaluation_settings` UI group selector/per-group listing+validation+audit। এর আগে EX-02 (roll-order ৬৫৩) ও EX-01 (Analysis subtab+import preserve ৬৩৮) merge হয়েছে। পূর্ণ suite এখন **৬৬৮ Django + ১৪ Node pass**, `check` 0, `makemigrations --check` clean (0044), `RetiredBoardFeatureTests` pass (SSC restore নয়)। Live Render/DB/backup এই sandbox থেকে **UNKNOWN** — docs ছাড়া নিশ্চিত দাবি নয়; কোনো production DB/credential ছোঁয়া হয়নি।
...
## 6. Test suite map (৬৬৮ Django + ১৪ Node — re-verified ২০২৬-০৯-২০, §20)

**Python (`manage.py test students`): 668 pass in ~253s** (৬২৫ baseline +১৩ EX-01 +১৫ EX-02 +১৫ EX-03; এই checkout `7a13e33`+0044·Δ=৬৬৮, ২০২৬-০৯-২০-এ নিজে চালানো)।
Isolation: 16 + 34 tests; Students/Admission/Exams/Attendance/HR/Finance/Backup/Media/SSC retirement as in previous §6 plus 72 backup tests (encryption/file modes/off-box stub/SHA/retention/health-gate), 33 media tests, +১৫ EX-01 `test_navigation`, +১৫ `test_register_roll_order`, +১৫ `test_group_based_mark_evaluation`।
...
| `python manage.py test students` | **`Ran 668 tests in ~253s` → `OK`** (৬২৫ +১৩ EX-01 +১৫ EX-02 +১৫ EX-03) |
