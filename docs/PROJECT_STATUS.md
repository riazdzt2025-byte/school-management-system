...
| E7 | চতুর্থ/অতিরিক্ত বিষয়ের GPA নিয়ম | **Complete** | **EX-06 policy correction (2026-09-23):** old automatic 4.90→5.00 boost removed. `result_utils.calculate_final_gpa()` applies `min(5.00, (main-point-sum + max(0, FOURTH-point−2.00)) / main-subject-count)` with deterministic two-place `ROUND_HALF_UP`. Only selected `Subject.category='FOURTH'` qualifies; ordinary OPTIONAL is main. Fourth F/AB = zero bonus but never fails the main result; a main F/AB still yields Fail/0.00. Total, percentage and rank tie-break use main subjects; final 5.00 is A+. One fourth choice per student is enforced before publishing (`fourth_subject_configuration_errors`). `test_fourth_subject_gpa.py` pins cap/no 5.20, fourth F versus main F, all result/print/analysis surfaces, and the publish block; `tests.py` pins no-fourth 4.90 stays 4.90. No result-specific GPA export endpoint exists. Full rule/examples: `RESULT_PUBLISHING_GUIDE.md` §2. Local suite 726/726 and Node 14/14 pass; PR #47 corrected-policy CI passed (sqlite 6m50s, Postgres 4m46s, Node 4s). |
