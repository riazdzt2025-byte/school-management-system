...
_Last updated: 2026-09-19 (সেশন EX-02 — প্রম্পট ০৩/২৮, register/roll-order নিয়ম; আগের এন্ট্রি: সেশন ০০ baseline যাচাই, প্রম্পট ০১/২৮)_
...
| E3 | Class Performance Register সহ result lists/print/export-এ numeric roll-order | **Complete** (EX-02, ২০২৬-০৯-১৯ — নিয়ম লিখিত + সব output-এ প্রয়োগ + ১৮ নতুন টেস্ট) | **নিয়ম:** register/roll output = numeric roll (`None` শেষে → name → pk); merit/rank output = `position` order (`top_10`, `full_rank_list`, `section_arrangement` — ইচ্ছাকৃত)। `result_sheet` (c17a45a) + **নতুন** `exam_result_summary` ও `result_analysis_result_cards` → `sort_result_rows_by_roll`; `student_list` / `download_student_list` → `F('roll_no').asc(nulls_last=True)` + `pk` (sqlite ও postgres-এ একই ক্রম; স্ক্রিন ও Excel এক); attendance class list name → roll (`roll_order_queryset`); seat-plan generation roll + `pk`; `get_exam_students` / `failed_subject_rows` / `section_arrangement_rows` → shared helper (`result_utils.roll_order_key` · `roll_order_queryset` · `sort_result_rows_by_roll`)। ৪টি print template-এ `thead{display:table-header-group}` + `tr{page-break-inside:avoid}`। Tests: `test_sheet_defaults_to_numeric_roll_order_without_changing_merit` + **নতুন `students/test_register_roll_order.py` (১৮ — roll 2/10/100/None, string-sort trap, merit-বিরোধী নম্বর, duplicate-roll stability, Excel-export ক্রম)** |
...
| **Complete** (এই সেশনে সম্পন্ন — ২০২৬-০৯-১৯) | **EX-02** (register/roll-order নিয়ম — `reports/EX-02.md`; PR OPEN, merge-অনুমোদন বাকি) |
| **Partial** (মূল অংশ আছে, নির্দিষ্ট কাজ বাকি) | **EX-01** (Analysis subtab — PR #39 merge-অনুমোদন বাকি), **EX-03** (group field), **OF-02** (১১টি list view), **OF-05** (ছবি), **OF-06** (progress page), **DB-02** (agent নামের comment), **DB-03** (SEC-FU-1/2), **AT-01** (correction), **EM-03** (closed-period) |
...
১. EX-01 Analysis subtab (PR #39 merge-অনুমোদন) ও ২. EX-03 `SubjectMarkSetting.group`
