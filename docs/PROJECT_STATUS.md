...
| E7 | Final GPA 4.90–4.99-কে 5.00 করার নিয়ম | **Complete** | **EX-06 (2026-09-23):** `result_utils.calculate_passing_gpa()` is the sole final-GPA helper. It averages subject points, `Decimal.quantize(Decimal('0.01'), ROUND_HALF_UP)` rounds deterministically, caps at `5.00`, then promotes the rounded interval `[4.90, 5.00)` to **5.00 / A+**. Owner confirmed on 2026-09-23: `4.895 → 4.90 → boost`, two-decimal display, and an otherwise passing student with an A/4.00 subject remains eligible. Fail/AB and No Marks never enter the helper; rank is calculated after the benefit. `test_gpa_boost.py` pins requested boundaries, a 5.00/4.95/4.89 cohort with positions 1/2/3, F/AB non-eligibility, and every existing result/view/print/analysis surface. No result-specific GPA export endpoint exists. Full rule/examples: `RESULT_PUBLISHING_GUIDE.md` §2. |
