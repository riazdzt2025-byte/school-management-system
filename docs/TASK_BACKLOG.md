...
_Last updated: 2026-09-16 (isolated verification `arena/01a0aaed-school-management-system`, base `158b98a` = `origin/main`)_
...
**Bengali TL;DR:** 2026-09-09 পর্যন্ত P0 isolation/validation/voucher/promotion/backup প্রায় সব শেষ; 2026-09-16 যাচাইয়ে 452 test pass, 0039-0042 guardian unification + result analysis + Full Rank List + row gating সব cover আছে। বাকি শুধু live Render ops (P0-7/ P0-8 live, P1-10 live switch, P1-11 bucket wiring) + দুটো বড় deferral (i18n, legacy drop) + doc tick (P0-9)। SSC restore করা হয়নি।
...
| P0-8 (tooling) | Backup/restore tooling + runbook + disposable drill | **complete** | `backup_data`/`restore_backup`/`check_backups`/`backup_cron.sh`/`render.cron.yaml` + `BACKUP_AND_RESTORE.md`; 14 backup tests; disposable drill byte-identical verified (historical) |
...
- **Tests:** existing 14 `test_backup_tooling.py` already cover `check_backups` exit codes; live is manual proof.
