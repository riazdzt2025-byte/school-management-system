...
**Bengali TL;DR:** 2026-09-09 পর্যন্ত P0 isolation/validation/voucher/promotion/backup প্রায় সব শেষ; 2026-09-16 যাচাইয়ে **510 test pass** (backup tooling-এ নতুন encryption / file-permission / off-box upload / media-SHA cover যোগ হয়েছে, ১৪ → ৭২ test), 0039-0042 guardian unification + result analysis + Full Rank List + row gating সব cover আছে। নতুন ডক: `docs/BACKUP_RESTORE_GUIDE.md` ও `docs/DATA_SAFETY_STATUS.md`; ড্রিল এখন `scripts/backup_smoke_test.sh` দিয়ে স্বয়ংক্রিয় (SQLite ১৫/১৫, PostgreSQL 16.2-এ ১৬/১৬ ধাপ pass; CI-তে দুটোই চলবে)। বাকি শুধু live Render ops (P0-7/ P0-8 live, P1-10 live switch, P1-11 bucket wiring) + দুটো বড় deferral (i18n, legacy drop) + doc tick (P0-9)। SSC restore করা হয়নি, লাইভ ব্যাকআপ চালু হয়নি।
...
- **Tests:** 72 tests in `test_backup_tooling.py` cover the backup/restore/health-gate paths including encryption, file modes, the `PG*` mapping and the (stubbed) off-box upload; `scripts/backup_smoke_test.sh` is the end-to-end drill and now runs in **both** CI jobs (sqlite; postgres against the `postgres:16` service). Locally: 15/15 (sqlite) and 16/16 (PostgreSQL 16.2) steps passed on disposable data. Live is still manual proof.
