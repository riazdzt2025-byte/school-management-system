...
# bucket:
#   * DATABASE_URL and MEDIA_ROOT are exported to paths inside the drill
#     directory, which overrides .env (python-dotenv does not overwrite
#     variables that are already set);
#   * before doing anything it asks Django which database it resolved and
#     ABORTS unless that path is inside the drill directory and the engine is
#     sqlite — so a stray production DATABASE_URL cannot be hit by accident;
...
#   scripts/backup_smoke_test.sh --keep     # keep the drill dir for inspection
#
...
# Disposable database + media, exported so they win over anything in .env.
...
case "$ENGINE" in
  *sqlite3*) ;;
  *) bad "engine is not sqlite ($ENGINE) — refusing to run a drill against it"; exit 1 ;;
esac
case "$DB_PATH" in
...
DATABASE_URL="sqlite:///$DST_DB" "$PY" manage.py restore_backup \
  --backup "$PLAIN_BACKUP" \
...
RESTORED_COUNT="$(DATABASE_URL="sqlite:///$DST_DB" "$PY" manage.py shell --command "
from students.models import Student
...
if command -v openssl >/dev/null 2>&1; then
  ...
  if [[ -f "$ENC_BACKUP/db.sqlite3" ]]; then
    bad "plaintext db.sqlite3 was left behind next to the ciphertext"
  else
  ...
  # Without the policy set, the gate must flag the same folder as non-compliant
  # only when encryption is REQUIRED — a plaintext policy must not fail here.
  rm -rf "$DRILL/restored-media-enc" && mkdir -p "$DRILL/restored-media-enc"
  BACKUP_ENCRYPTION=openssl BACKUP_PASSPHRASE_FILE="$KEY_FILE" \
  DATABASE_URL="sqlite:///$DRILL/dst-enc.sqlite3" \
  ...
  if BACKUP_ENCRYPTION=openssl BACKUP_PASSPHRASE_FILE="$WRONG_KEY" \
     DATABASE_URL="sqlite:///$DRILL/dst-wrong.sqlite3" \
     "$PY" manage.py restore_backup \
